...
from src.utils import setuplogging, init_process, cleanup_process, init_config, dump_args
from src.dataloader import DataLoaderTest
from src.speedyfeed import SpeedyFeed

from src.utils import ndcg_score, mrr_score, ctr_score
...
def test_auc(local_rank,news_index,news_vecs, args):
    ...
    for cnt, (log_vecs, log_mask, news_vecs, labels) in tqdm(enumerate(dataloader)):
        ...
        if args.world_size > 1:
            user_vecs = ddp_model.module.user_encoder.infer_user_vec(log_vecs, log_mask).to(torch.device("cpu")).detach().numpy()
        else:
            user_vecs = ddp_model.user_encoder.infer_user_vec(log_vecs, log_mask).to(torch.device("cpu")).detach().numpy()
